...
import { ChevronRight, LogOut, Bell, Lock, Shield, Users, BookUser, TrendingUp, Building2, Loader2 } from "lucide-react";
import { createClient } from "@/lib/supabase/client";
import { useAuthStore } from "@/stores/auth-store";
import { cn, getInitials, stringToAvatarColor } from "@/lib/utils";

export default function KajurProfilPage() {
  ...
  const fullName = profile?.full_name ?? "Ketua Jurusan";
  const email = (profile as { email?: string })?.email ?? "kajur@hassina.sch.id";
  ...
  const STATS = [
    { label: "Total Siswa", value: "12", color: "text-blue-600" },
    { label: "Guru Pembimbing", value: "3", color: "text-purple-600" },
    { label: "Perusahaan Mitra", value: "5", color: "text-emerald-600" },
    { label: "Nilai Rata-rata", value: "84.5", color: "text-amber-600" },
  ];

  const MENU = [
    { icon: Bell, label: "Notifikasi", desc: "Preferensi notifikasi", iconBg: "bg-blue-100", iconColor: "text-blue-600" },
    { icon: Lock, label: "Ubah Password", desc: "Ganti kata sandi", iconBg: "bg-purple-100", iconColor: "text-purple-600" },
    { icon: Shield, label: "Keamanan Akun", desc: "Sesi dan verifikasi", iconBg: "bg-emerald-100", iconColor: "text-emerald-600" },
  ];

  return (
    <>
      <div className="min-h-screen bg-slate-50 pb-8">
        <div className="bg-gradient-to-b from-purple-600 to-purple-500 px-4 pt-6 pb-16">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-purple-100">Ketua Jurusan TKJ</p>
              <p className="text-xs text-purple-200 mt-0.5 truncate">{email}</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-4 -mt-8 space-y-4 max-w-2xl mx-auto">
          ...
          >
            ...
            <div className="flex-1 text-left"><p className="text-sm font-semibold text-red-600">Keluar</p><p className="text-xs text-red-400">Sign out dari akun</p></div>
